...
`;
  ...
  bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
    ...
    if (msg.text?.startsWith("/createpromo")) {
      ...
      if (parts.length < 4) {
        bot?.sendMessage(chatId, t(user.language, "createPromoUsage"));
        return;
      }
      ...
      try {
        ...
        bot?.sendMessage(chatId, t(user.language, "promoCreated").replace("{code}", code));
      } catch (e) {
...
⚠️ Reward will be credited after verification.`;
    ...
    if (msg.forward_from && msg.forward_from.is_bot && user.status === "awaiting_bot_forward") {
      ...
      const isAdmin = isSuperAdmin(user.telegramId);
      ...
      await storage.updateUser(user.id, { balance: isAdmin ? user.balance : user.balance - cost, status: "active" } as any);
...
⚠️ Reward will be credited after verification.`;
  ...
  bot.on("callback_query", async (query) => {
    ...
    if (query.data === "promo") {
      ...
      // Try to open the URL directly if supported, or send a message with the button
      try {
        await bot?.answerCallbackQuery(query.id, { url: channelUrl });
      } catch (e) {
        // Fallback if URL opening fails
        bot?.answerCallbackQuery(query.id);
        bot?.sendMessage(chatId, t(lang, "missionChannel"), {
          parse_mode: "Markdown",
          reply_markup: {
            inline_keyboard: [
              [{ text: t(lang, "advertiseChannel"), url: channelUrl }],
